...
export default async function handler(req: any, res: any) {
  console.log(`[revalidation] start: Start. Request: ${JSON.stringify(req.body)}`);
  ...
  try {
    const layoutUpdates = req.body.updates.filter(
      (update: any) => update.entity_definition == 'LayoutData'
    );
